...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetchFormResponses(); // Sincronizar respuestas al iniciar
});
...
function initializeEventListeners() {
    ...
    document.addEventListener("click", function (event) {
        ...
        if (header && dropdown && !header.contains(event.target) && !dropdown.contains(event.target)) {
            ...
            <div class="industrial-item">
                <div>
                    ...
                    <div class="industrial-specialties">
                        ${industrial.specialties.map(specialty => 
...
function handleIncidentSubmit(e) {
    ...
    if (!community) {
        showNotification("Por favor, seleccione una comunidad válida", "error");
        return;
    }
...
function syncWithGoogleSheets(incident) {
    const googleSheetsUrl = "https://script.google.com/macros/s/AKfycbw1kI7k-XYU_DDp_PuioDp5RpG1sUaw03Xu8vb6mft9_AYFSSiKArnciAitO0IfELOQ/exec";
...
function sendIncidentNotificationToIndustrials(incident) {
    incident.industrials.forEach((industrial, index) => {
            ...
            <div class="incident-item">
                <div class="incident-info">
                    ...
                    <div class="industrials-assigned">
                        ${incident.industrials.map(industrial => 
                            `<span class="industrial-mini-tag">${industrial.name}</span>`
                        ).join("")}
                    </div>
...
function handleScheduleSubmit(e) {
    ...
    if (!incident) {
        showNotification("Error: Incidencia no encontrada", "error");
        return;
    }
    ...
    sendVisitConfirmationToCommunity(visit);
    if (sendReminder) {
        ...
        .map(visit => `
            <div class="incident-item">
                <div class="incident-info">
                    ...
                    <p><strong>Industrial(es):</strong></p>
                    <div class="industrials-assigned">
                        ${visit.incident.industrials.map(industrial => 
                            `<span class="industrial-mini-tag">${industrial.name}</span>`
                        ).join("")}
                    </div>
...
function handleUpdateVisitSubmit(e) {
    ...
    if (!visit || !incident) {
        showNotification("Error: Visita o incidencia no encontrada", "error");
        return;
    }
    ...
    if (newStatus === "completed") {
        sendVisitCompletionToCommunity(visit);
...
function updateStats() {
    const elements = {
        ...
        assignedIncidents: document.getElementById("assignedIncidents"),
        completedIncidents: document.getElementById("completedIncidents"),
        totalCommunities: document.getElementById("totalCommunities"),
        totalIndustrials: document.getElementById("totalIndustrials")
    };
    
    if (elements.totalIncidents) elements.totalIncidents.textContent = incidents.length;
    if (elements.pendingIncidents) elements.pendingIncidents.textContent = incidents.filter(inc => inc.status === "pending").length;
    if (elements.assignedIncidents) elements.assignedIncidents.textContent = incidents.filter(inc => inc.status === "scheduled").length;
    if (elements.completedIncidents) elements.completedIncidents.textContent = incidents.filter(inc => inc.status === "completed").length;
    if (elements.totalCommunities) elements.totalCommunities.textContent = communities.length;
    if (elements.totalIndustrials) elements.totalIndustrials.textContent = industrials.length;
}
...
function closeUpdateModal() {
    const modal = document.getElementById("updateVisitModal");
